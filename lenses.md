...
```
Получается чуть лаконичнее - однако лаконичность это не главное. Главное это композируемость.
...
```
И теперь магия!
Чтобы получить линзу для from у rangeSlider нужно просто скомпозировать две уже опеределенных нами линзы:
```Javascript
